...
def main():
    ...
    # 判断SQL模板是否包含时间通配符
    has_time_placeholder = ("{start_time}" in sql_template) or ("{end_time}" in sql_template)

    if has_time_placeholder:
        # 检查参数
        if not args.start_date or not args.end_date:
            logger.error("SQL模板包含{start_time}或{end_time}，但未提供--start-date和--end-date参数")
            raise ValueError("请提供--start-date和--end-date参数")

        # 解析日期
        start = datetime.strptime(args.start_date, '%Y-%m-%d')
        end = datetime.strptime(args.end_date, '%Y-%m-%d')
        logger.info(f"查询时间范围: {args.start_date} 至 {args.end_date}")
        ...
        while current_start <= end:
            # 计算当前批次的结束日期
            current_end = current_start + timedelta(days=args.interval_days)
            if current_end > end:
                current_end = end

            # 格式化日期为字符串
            start_str = current_start.strftime('%Y-%m-%d 00:00:00')
            end_str = current_end.strftime('%Y-%m-%d 23:59:59')
        ...
        # 没有时间通配符，直接查询一次
        logger.info("SQL模板不包含{start_time}或{end_time}，将直接执行一次查询")
        try:
            result = query_sql_by_date_range(
                api_key,
                args.project,
            ...
            save_to_csv(result, args.output, False, logger)
            logger.info(f"任务完成，数据已保存到 {args.output}")
        except Exception as e:
            logger.error(f"执行SQL时出错: {e}", exc_info=True)
