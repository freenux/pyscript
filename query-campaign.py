...
def query_appsflyer_ids_from_login_id(cursor, login_id: str = None) -> List[str]:
    """
...
def query_appsflyer_ids_from_device_id(cursor, device_id: str = None) -> List[str]:
    """
    查询设备对应的用户appsflyer_id
...
def query_campaign_in_database(cursor, apps_flyer_ids: List[str], logger: logging.Logger) -> List[dict]:
    ...
    with open(input_file, 'r', encoding='utf-8') as f_in, open(output_file, 'w', encoding='utf-8') as f_out:
        ...
        for row in reader:
            ...
            if is_login_id(second_id):
                login_id = second_id
                apps_flyer_ids = query_appsflyer_ids_from_login_id(cursor_xs, login_id=login_id)
            ...
            if len(campaign_result) == 0:
                ...
                apps_flyer_ids = query_appsflyer_ids_from_device_id(cursor_xs, device_id=device_id)
                if len(apps_flyer_ids) > 0:
                    campaign_result = query_campaign_in_database(cursor_promotional, apps_flyer_ids, logger)

            if len(apps_flyer_ids) == 0:
                logger.info(f'Not found appsflyer_id for this user: {row}')
                continue

            if len(campaign_result) == 0:
                logger.info(f'Not found campaign data for this user, apps_flyer_ids: {apps_flyer_ids}, row: {row}')
                continue
