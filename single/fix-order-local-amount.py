...
def update_prepaid_order(conn, cursor, order_id, old_local_amount, new_local_amount, fix_reason, logger, debug=False):
    ...
    if not debug:
        cursor.execute(update_query, (new_local_amount, order_id, old_local_amount))
        conn.commit()
    logger.info(f"SQL: {update_query} with params {(new_local_amount, order_id, old_local_amount)}")
    logger.info(f"Updated order {order_id}: {old_local_amount} -> {new_local_amount} ({fix_reason})")

def update_order(conn, cursor, user_id, order_id, old_local_amount, new_local_amount, fix_reason, logger, debug):
...
def process_orders(prepaid_conn, order_conn, start_time, end_time, d1, d2, d3, logger, geo_reader, debug=False):
    """Process orders within the specified time range."""
    prepaid_cursor = prepaid_conn.cursor()
    ...
    try:
        ...
        """
        prepaid_cursor.execute(query, (start_time, end_time))
        orders = prepaid_cursor.fetchall()
        ...
        for order in orders:
            ...
            if fixed_amount and fixed_amount != order['local_amount']:
                # update_prepaid_order(prepaid_conn, prepaid_cursor, order['id'], order['local_amount'], fixed_amount, fix_reason, logger, debug)
                update_order(order_conn, order_cursor, order['qid'], order['id'], order['local_amount'],fixed_amount, fix_reason, logger, debug)
                updates += 1
        ...
        # prepaid_conn.rollback()
        order_conn.rollback()
    finally:
        # prepaid_cursor.close()
        order_cursor.close()
...
def main():
    ...
    parser.add_argument('--prepaid-dsn', required=True, help="DSN name in mycli config")
    parser.add_argument('--order-dsn', required=True, help="DSN name in mycli config")
    ...
    try:
        ...
        prepaid_db_config = parse_mycli_config(args.mycli_config, args.prepaid_dsn)
        order_db_config = parse_mycli_config(args.mycli_config, args.order_dsn)
        ...
        # Connect to database
        prepaid_conn = pymysql.connect(**prepaid_db_config)
        order_conn = pymysql.connect(**order_db_config)
        
        try:
            # Process orders
            process_orders(prepaid_conn, order_conn, args.start_time, args.end_time, d1, d2, d3, logger, geo_reader, args.debug)
        finally:
            prepaid_conn.close()
            order_conn.close()
